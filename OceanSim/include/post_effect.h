...
public:
    ...
    void Update(float dt, glm::vec3 center_pos, glm::vec3 camera_pos) 
    {
        ...
        flame_1.Update(dt, center_pos, (time_ > 3) ? 0 : new_particles_flame1, glm::vec3(0), camera_pos);

        frag_1.Update(dt, center_pos, (frag_cnt != 0 || time_ > 3) ? 0 : FRAG_NUM, glm::vec3(0), camera_pos);
...
public:
    void explode_init();
    void begin_explode() { explode_begin_ = true; explode_init(); };
    void Update(float dt, glm::vec3 center_pos, glm::vec3 camera_pos);
    void Draw(glm::mat4 view_mat, glm::mat4 proj_mat);
};
